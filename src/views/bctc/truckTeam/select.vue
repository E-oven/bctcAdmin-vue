<template>
    <avue-crud :option="option"
        ...
        <template slot="teamStatus" slot-scope="scope">
          <el-button type="success" size="mini" v-if="scope.row.teamStatus == 'F'">空 闲</el-button>
          <el-button type="danger" size="mini" v-if="scope.row.teamStatus == 'B'">繁 忙</el-button>
        </template>
  ...
  export default {
    ...
    methods: {
      handleAdd() {
        ...
        if(this.selectionList.some(it=>it.teamStatus == 'B')){
          this.$message.warning("选择记录中包含繁忙拖车组")
          return
        }
