...
<script>
  import {getList, getDetail, add, update, remove, isExists} from "./api.js";
  ...
  export default {
    ...
    computed: {
      ...
      option2(){
        let opt = option
        opt.column.filter(it=>it.prop == 'number')[0].rules.push({ validator: this.validateNumber, trigger: 'blur' })
        return opt
      }
    },
    methods: {
      async validateNumber(rule, value, callback) {
          const resp = await isExists(this.form.id, value)
          console.log('isExists', resp)
          if (resp.data.data) {
