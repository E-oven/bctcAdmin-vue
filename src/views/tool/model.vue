        ...
        if (result.success) {
          ...
          this.fields.forEach(item => {
            ...
            if (!validatenull(item.name)) {
              ...
              // 根据字段物理类型自动适配实体类型
              if (item.propertyType === "LocalDateTime" || item.propertyType === 'LocalDate') {
                item.propertyType = "Date";
                item.propertyEntity = "java.util.Date";
              }else if (item.propertyType === "Object" && item.metaInfo.jdbcType == 'DOUBLE') {
                item.propertyType = "Double";
                item.propertyEntity = "Double";
              } else {
                entityDic.forEach(d => {
